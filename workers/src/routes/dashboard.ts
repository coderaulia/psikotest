    ...
    return c.json({
      ...
      recentParticipants: (recentRows.results ?? []).map((r) => {
        ...
        return {
          ...
          summary: row.latest_status === 'scored' ? 'Completed scoring' : 'Finished test',
        };
      }),
    });
  } catch (error) {
    // If table properties don't exist yet, return empty arrays gracefully
    return c.json({
      summaryCards: [
        { label: 'Active Sessions', value: String(activeSessions), sublabel: `${draftSessions} draft` },
        { label: 'Participants', value: String(participantCount), sublabel: null },
        { label: 'Completed Tests', value: String(completedSubmissions), sublabel: '0% completion rate' },
        { label: 'Avg IQ Score', value: averageIqScore != null ? String(averageIqScore) : '—', sublabel: null },
      ],
      metrics: { activeSessions, draftSessions, participantCount, totalSubmissions, completedSubmissions, averageIqScore },
      distributions: { disc: [], workload: [] },
      liveSessions: [],
      recentParticipants: [],
    });
  }
...
app.get('/summary', getDashboardData);
