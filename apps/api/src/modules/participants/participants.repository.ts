...
export interface ParticipantListItem {
  ...
  latestTestType: PublicTestTypeCode | null;
  latestStatus: 'not_started' | 'in_progress' | 'submitted' | 'scored';
  totalSubmissions: number;
  lastActivityAt: string | null;
}

interface ParticipantListRow extends RowDataPacket {
  id: number;
  full_name: string;
  email: string;
  ...
  latest_test_type: PublicTestTypeCode | null;
  latest_status: 'not_started' | 'in_progress' | 'submitted' | 'scored' | null;
...
export async function fetchParticipants(filters: ParticipantListFilters = {}) {
  ...
  const [rows] = await pool.query<ParticipantListRow[]>(
    `
      SELECT
        p.id,
        p.full_name,
