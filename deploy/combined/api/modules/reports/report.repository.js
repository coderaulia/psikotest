...
function normalizeReviewStatus(rawStatus, releasedAt) {
    if (releasedAt) {
        return 'released';
    }
...
function formatDistributionLabel(status) {
    switch (status) {
        case 'released':
            return 'Released';
        case 'reviewed':
...
export async function fetchReportOverviewCounts() {
    ...
    const [[resultRows], [participantRows], [sessionRows]] = await Promise.all([
        pool.query('SELECT COUNT(*) AS value FROM results'),
        pool.query('SELECT COUNT(*) AS value FROM participants'),
        pool.query("SELECT COUNT(*) AS value FROM test_sessions WHERE status IN ('active', 'completed')"),
    ]);
    return {
