...
siteResultsRoutes.get('/export.csv', asyncHandler(async (request, response) => {
...
siteResultsRoutes.get('/:id', asyncHandler(async (request, response) => {
    if (!request.customerSession) {
        throw new HttpError(401, 'Customer session is required');
    }
...
siteResultsRoutes.get('/', asyncHandler(async (request, response) => {
    if (!request.customerSession) {
        throw new HttpError(401, 'Customer session is required');
    }
