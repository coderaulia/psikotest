...
import { addCustomerWorkspaceMember, getCustomerWorkspaceActivity, getCustomerWorkspaceSettings, listCustomerWorkspaceMembers, sendCustomerWorkspaceMemberInvite, updateCustomerWorkspaceSettings, } from './site-workspace.service.js';
...
siteWorkspaceRoutes.get('/settings', asyncHandler(async (request, response) => {
...
siteWorkspaceRoutes.patch('/settings', asyncHandler(async (request, response) => {
    requireCustomerWorkspaceRole(request, ['owner', 'admin'], 'Workspace settings are limited to owners and workspace admins');
...
siteWorkspaceRoutes.get('/activity', asyncHandler(async (request, response) => {
    if (!request.customerSession) {
        throw new HttpError(401, 'Customer session is required');
    }
...
siteWorkspaceRoutes.get('/team', asyncHandler(async (request, response) => {
    if (!request.customerSession) {
        throw new HttpError(401, 'Customer session is required');
    }
...
siteWorkspaceRoutes.post('/team', asyncHandler(async (request, response) => {
...
siteWorkspaceRoutes.post('/team/:memberId/send', asyncHandler(async (request, response) => {
    requireCustomerWorkspaceRole(request, ['owner', 'admin'], 'Workspace team management is limited to owners and workspace admins');
