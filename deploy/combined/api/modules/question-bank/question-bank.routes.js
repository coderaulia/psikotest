...
import { createQuestionBankQuestion, getQuestionBankQuestionById, listQuestionBankQuestions, updateQuestionBankQuestion, } from './question-bank.service.js';
...
questionBankRoutes.get('/questions', asyncHandler(async (request, response) => {
    const filters = querySchema.parse(request.query);
    response.json({ items: await listQuestionBankQuestions(filters) });
}));
questionBankRoutes.get('/questions/:id', asyncHandler(async (request, response) => {
...
questionBankRoutes.post('/questions', asyncHandler(async (request, response) => {
...
questionBankRoutes.patch('/questions/:id', asyncHandler(async (request, response) => {
    if (!request.adminSession) {
        throw new HttpError(401, 'Admin session is required');
    }
    const payload = questionSchema.parse(request.body);
