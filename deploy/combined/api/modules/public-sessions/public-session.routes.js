...
import { getPublicSession, getSubmissionQuestionWindow, saveSubmissionAnswers, startPublicSubmission, submitPublicSubmission, } from './public-session.service.js';
...
function readSubmissionAccessToken(authorizationHeader, customHeader) {
...
publicSessionRoutes.get('/session/:token', sessionLookupRateLimit, asyncHandler(async (request, response) => {
    const token = String(request.params.token);
    response.json(await getPublicSession(token));
}));
publicSessionRoutes.post('/session/:token/start', sessionStartRateLimit, asyncHandler(async (request, response) => {
    const payload = startSchema.parse(request.body);
    const token = String(request.params.token);
    response.status(201).json(await startPublicSubmission(token, payload));
}));
publicSessionRoutes.get('/submissions/:submissionId/questions', submissionWriteRateLimit, asyncHandler(async (request, response) => {
    const query = questionWindowQuerySchema.parse(request.query);
    const submissionId = Number(request.params.submissionId);
    const submissionAccessToken = readSubmissionAccessToken(request.header('Authorization') ?? undefined, request.header('X-Submission-Token') ?? undefined);
    response.json(await getSubmissionQuestionWindow(submissionId, submissionAccessToken, query.groupIndex ?? 0));
}));
publicSessionRoutes.post('/submissions/:submissionId/answers', submissionWriteRateLimit, asyncHandler(async (request, response) => {
...
publicSessionRoutes.post('/submissions/:submissionId/submit', submissionFinalizeRateLimit, asyncHandler(async (request, response) => {
