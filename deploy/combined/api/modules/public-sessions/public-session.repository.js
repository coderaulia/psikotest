...
import { getParticipantResultMode, parseTestSessionSettings, } from '../test-sessions/session-settings.js';
...
function validateSubmissionAnswers(questions, answers) {
    ...
    for (const answer of answers) {
        ...
        const assertOptionId = (optionId, label) => {
...
async function countAnsweredQuestions(executor, submissionId) {
...
async function loadSessionRowByToken(token) {
...
async function loadSessionRowById(sessionId) {
    const pool = getDbPool();
    const [rows] = await pool.query(`
      SELECT
        ts.id AS session_id,
...
async function loadQuestionsByTestType(testTypeId) {
    const pool = getDbPool();
...
function buildSessionDefinition(sessionRow, questions) {
...
async function loadSubmissionContextRow(submissionId) {
    const pool = getDbPool();
    const [rows] = await pool.query(`
      SELECT
...
async function loadStoredAnswers(submissionId) {
    const pool = getDbPool();
...
export async function findPublicSessionByToken(token) {
...
async function resolveParticipantId(connection, participant) {
...
export async function createSubmissionForToken(token, participant) {
    const sessionRow = await loadSessionRowByToken(token);
    if (!sessionRow) {
        return null;
    }
    ...
    try {
      ...
      `, [
        ...
        return {
            submissionId: Number(insertResult.insertId),
            participantId,
            token,
            answerSequence: 0,
            status: 'in_progress',
...
export async function replaceSubmissionAnswers(submissionId, answers, answerSequence) {
    const pool = getDbPool();
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
      ...
      `, [submissionId]);
        ...
        await connection.query(`DELETE FROM answers WHERE submission_id = ? AND question_id IN (${placeholders})`, [submissionId, ...answeredQuestionIds]);
        for (const answer of answers) {
...
export async function loadSubmissionQuestionWindow(submissionId, groupIndex) {
...
export async function loadSubmissionScoringContext(submissionId) {
    const contextRow = await loadSubmissionContextRow(submissionId);
    if (!contextRow) {
        return null;
    }
