...
import { assignResultReviewer, getResultById, getReviewerQueueSummary, listResults, listReviewerOptions, listReviewerQueue, updateResultReview, } from './result.service.js';
...
function requireReviewerSession(request) {
...
resultRoutes.get('/', asyncHandler(async (request, response) => {
    const filters = querySchema.parse(request.query);
    response.json({ items: await listResults(filters) });
}));
resultRoutes.get('/reviewers', asyncHandler(async (request, response) => {
    requireAdminSession(request);
    response.json({ items: await listReviewerOptions() });
}));
resultRoutes.get('/reviewer-queue/summary', asyncHandler(async (request, response) => {
    const adminSession = requireReviewerSession(request);
    response.json(await getReviewerQueueSummary(adminSession));
}));
resultRoutes.get('/reviewer-queue', asyncHandler(async (request, response) => {
    const adminSession = requireReviewerSession(request);
...
resultRoutes.get('/:id', asyncHandler(async (request, response) => {
...
resultRoutes.patch('/:id/assign-reviewer', asyncHandler(async (request, response) => {
    const adminSession = requireAdminSession(request);
...
resultRoutes.patch('/:id/review', asyncHandler(async (request, response) => {
...
resultRoutes.patch('/:id/review-status', asyncHandler(async (request, response) => {
