...
import { acceptWorkspaceInvite, getCustomerSessionProfile, getWorkspaceInvitePreview, loginCustomer, logoutCustomer, signupCustomer, } from './site-auth.service.js';
...
siteAuthRoutes.post('/signup', signupRateLimit, asyncHandler(async (request, response) => {
    const payload = signupSchema.parse(request.body);
    const session = await signupCustomer(payload);
    response.status(201).json(session);
}));
siteAuthRoutes.post('/login', loginRateLimit, asyncHandler(async (request, response) => {
...
siteAuthRoutes.get('/team-invites/:token', asyncHandler(async (request, response) => {
...
siteAuthRoutes.post('/team-invites/:token/accept', inviteAcceptRateLimit, asyncHandler(async (request, response) => {
    const { token } = inviteParamsSchema.parse(request.params);
...
siteAuthRoutes.post('/logout', requireCustomerAuth, asyncHandler(async (request, response) => {
...
siteAuthRoutes.get('/me', requireCustomerAuth, asyncHandler(async (request, response) => {
    if (!request.customerSession) {
        throw new HttpError(401, 'Customer session is required');
    }
