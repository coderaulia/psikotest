...
function getPreviewDemoToken(testType) {
    if (testType === 'disc') {
        return 'disc-batch-a';
    }
    if (testType === 'iq') {
...
function mapResultVisibility(settings) {
...
function mapAssessmentDetailRow(row) {
    const settings = parseTestSessionSettings(row.settings_json);
...
function getAssessmentSelectSql(whereClause) {
    return `
      SELECT
...
export async function resolveTestTypeId(testType) {
    const pool = getDbPool();
...
export async function fetchCustomerAssessmentBySessionId(customerAccountId, sessionId) {
    const pool = getDbPool();
    const [rows] = await pool.query(`${getAssessmentSelectSql('WHERE ca.customer_account_id = ? AND ca.test_session_id = ? LIMIT 1')}`, [customerAccountId, sessionId]);
    const row = rows[0];
    return row ? mapAssessmentRow(row) : null;
}
export async function fetchCustomerAssessmentById(customerAccountId, assessmentId) {
    const pool = getDbPool();
    const [rows] = await pool.query(`${getAssessmentSelectSql('WHERE ca.customer_account_id = ? AND ca.id = ? LIMIT 1')}`, [customerAccountId, assessmentId]);
    const row = rows[0];
    return row ? mapAssessmentDetailRow(row) : null;
}
export async function fetchCustomerAssessments(customerAccountId) {
    const pool = getDbPool();
    const [rows] = await pool.query(`${getAssessmentSelectSql('WHERE ca.customer_account_id = ? ORDER BY ca.created_at DESC, ca.id DESC')}`, [customerAccountId]);
...
export async function activateCustomerAssessmentRecord(customerAccountId, assessmentId) {
    const pool = getDbPool();
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
...
function mapSubmissionStatusToParticipantStatus(invitationStatus, submissionStatus) {
...
async function fetchParticipantSubmissionMatches(sessionId, emails) {
    if (emails.length === 0) {
        return new Map();
    }
...
export async function fetchCustomerAssessmentParticipants(customerAccountId, assessmentId) {
    ...
    `, [assessmentId]);
    const submissionMap = await fetchParticipantSubmissionMatches(assessment.sessionId, rows.map((row) => row.email));
