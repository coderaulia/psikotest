...
import { FakeDbPool } from './support/fake-db.js';
function createFakeState(passwordHash) {
    return {
        admins: [
            {
                id: 1,
...
export async function runApiIntegrationTests() {
    ...
    try {
        ...
        assert.equal(loginPayload?.admin && typeof loginPayload.admin === 'object' ? loginPayload.admin.email : null, 'admin@example.com');
        ...
        assert.equal(billingOverviewPayload?.subscription && typeof billingOverviewPayload.subscription === 'object' ? billingOverviewPayload.subscription.planCode : null, 'research');
        assert.equal(billingOverviewPayload?.usage && typeof billingOverviewPayload.usage === 'object' ? billingOverviewPayload.usage.teamSeatCount : null, 1);
        assert.equal(Array.isArray(billingOverviewPayload?.diagnostics), true);
        assert.equal(billingOverviewPayload?.upgradeGuidance && typeof billingOverviewPayload.upgradeGuidance === 'object' ? billingOverviewPayload.upgradeGuidance.isUpgradeRecommended : null, false);
        ...
        assert.equal(billingCheckoutPayload?.checkoutSession && typeof billingCheckoutPayload.checkoutSession === 'object' ? billingCheckoutPayload.checkoutSession.status : null, 'open');
        ...
        assert.equal(billingUpdatePayload?.subscription && typeof billingUpdatePayload.subscription === 'object' ? billingUpdatePayload.subscription.planCode : null, 'starter');
        ...
        assert.equal((billingUpdatePayload?.recentInvoices?.length ?? 0) > 0, true);
        ...
        assert.equal(sessionPayload?.session && typeof sessionPayload.session === 'object' ? sessionPayload.session.title : null, 'DISC Hiring Session');
        assert.equal(sessionResponse.headers.get('cache-control'), 'no-store, no-cache, must-revalidate, private');
        assert.equal(Array.isArray(sessionPayload?.questions) ? (sessionPayload?.questions).length : -1, 0);
        assert.equal(sessionPayload?.session && typeof sessionPayload.session === 'object' ? sessionPayload.session.delivery.mode : null, 'progressive');
        ...
        const windowQuestions = Array.isArray(questionWindowPayload?.questions) ? questionWindowPayload.questions : [];
        const windowOptions = windowQuestions[0] && Array.isArray(windowQuestions[0].options) ? windowQuestions[0].options : [];
        ...
        assert.equal(typeof result?.resultPayload?.note, 'string');
