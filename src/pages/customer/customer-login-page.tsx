...
const copy = {
  en: {
    ...
    heroTitle: 'Build a compliant assessment draft before you share.',
    heroDescription:
    ...
    description: 'For workspace owners, operators, reviewers, and research teams using the self-serve SaaS flow.',
    fields: {
      email: 'Email',
      password: 'Password',
    },
    ...
    resetSuccess: 'Your password has been reset. You can sign in with the new password now.',
    errors: {
      fallback: 'Unable to sign in',
    },
  },
  id: {
    badge: 'Akses pelanggan',
    heroTitle: 'Susun draft asesmen yang patuh sebelum dibagikan.',
    heroDescription:
      'Masuk sebagai pemilik workspace atau anggota tim untuk mengatur asesmen, meninjau alur peserta, dan mengoperasikan workspace yang sama.',
    title: 'Login workspace',
    description: 'Untuk pemilik workspace, operator, reviewer, dan tim riset yang menggunakan alur SaaS mandiri.',
    fields: {
      email: 'Email',
      password: 'Kata sandi',
    },
    actions: {
      submit: 'Buka workspace',
      submitting: 'Sedang masuk...',
      createAccount: 'Buat akun',
      webmasterLogin: 'Login webmaster',
      forgotPassword: 'Lupa kata sandi?',
    },
    resetSuccess: 'Kata sandi Anda berhasil diperbarui. Sekarang Anda bisa masuk dengan kata sandi baru.',
    errors: {
      fallback: 'Tidak dapat masuk',
    },
  },
} as const;
        ...
        <Card className="grid w-full overflow-hidden border-white/80 bg-white/82 md:grid-cols-[0.96fr_1.04fr]">
          ...
          <div className="p-8 sm:p-10">
            ...
            <CardContent className="mt-8 p-0">
              {resetSuccess ? (
                <div className="mb-5 rounded-3xl border border-emerald-100 bg-emerald-50/80 p-4 text-sm leading-7 text-emerald-950/85">
                  {t.resetSuccess}
                </div>
              ) : null}
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div className="space-y-2">
                  <label htmlFor="customer-email" className="text-sm font-medium text-slate-600">{t.fields.email}</label>
                  <Input id="customer-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" />
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-4">
                    <label htmlFor="customer-password" className="text-sm font-medium text-slate-600">{t.fields.password}</label>
                    <Link to="/forgot-password" className="text-sm font-medium text-slate-500 underline-offset-4 hover:text-slate-950 hover:underline">{t.actions.forgotPassword}</Link>
                  </div>
              ...
              <div className="mt-6 flex flex-col gap-3 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                <Link to="/signup" className="font-medium text-slate-950">{t.actions.createAccount}</Link>
