...
const navItems = [
  ...
  { to: '/workspace/create', label: 'Create Assessment' },
];
      ...
      <header className="sticky top-0 z-20 border-b border-white/60 bg-white/72 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-lg font-semibold">Customer workspace</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden rounded-full border border-slate-200 bg-white/90 px-4 py-2 text-sm text-slate-500 lg:block">
              {customerSession?.account.organizationName ?? customerSession?.account.email ?? 'Workspace'}
            </div>
            <button
              type="button"
              onClick={() => {
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <div className="mb-8 grid gap-4 rounded-[32px] border border-white/70 bg-white/74 p-6 shadow-panel backdrop-blur lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div className="space-y-3">
            ...
            <h1 className="text-3xl font-semibold tracking-tight">Create assessment drafts before you publish or upgrade.</h1>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-slate-50/80 p-5">
            <p className="text-sm font-medium text-slate-950">Current customer access</p>
            ...
            <Link to="/workspace/create" className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-slate-950">
              Create a new draft <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
